...
#include <iostream>
#include <vector>
#include <algorithm> // For make_heap, push_heap
using namespace std;
...
void insertMinHeap(vector<int>& heap, int value) {
    ...
    push_heap(heap.begin(), heap.end(), greater<int>()); // Maintain Min-Heap property
}
...
int main() {
    ...
    vector<int> heap;
    make_heap(heap.begin(), heap.end(), greater<int>()); // Initialize as Min-Heap

    for (int i = 0; i < n; i++) {
        int age;
        cin >> age;
        insertMinHeap(heap, age); // Insert each element into the Min-Heap
    }

    cin >> threshold;

    // Print the Min-Heap array representation (unsorted)
    for (int i = 0; i < heap.size(); i++) {
        cout << heap[i] << " ";
    }
    cout << endl;

    // Count participants whose ages are less than the threshold
    int count = 0;
    for (int age : heap) {
        if (age < threshold) {
            count++;
