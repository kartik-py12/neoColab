...
void traverseInventory(const vector<int>&arr) {
...
void insertItem(vector<int>&arr,int item) {
    //Type your code here
    cout<<"Inserting Item: "<<item<<endl;
    arr.push_back(item);
    traverseInventory(arr);
    
}

void removeItem(vector<int>&arr,int item) {
    //Type your code here
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        insertItem(arr, newItems[i]);
    }

    cin>>m;
    deleteItems.resize(m);
    for (int i = 0; i < m; i++) {
        cin>>deleteItems[i];
    }
    
    for (int i = 0; i < m; i++) {
        removeItem(arr, deleteItems[i]);
    }
