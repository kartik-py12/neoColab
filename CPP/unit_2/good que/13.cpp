...
class BankATM{
    private:
        ...
        float dep;
    public:
        BankATM(float &a,float w,float d):aval(a),with(w),dep(d) {}
        void withdraw();
        void deposit(){
            aval+=dep;
            cout<<"Deposit successful. Available cash: "<<fixed<<setprecision(1)<<aval<<endl;
        }
        
        void cash(){
            cout<<"Available cash in ATM: "<<aval;
        }
};

void BankATM::withdraw(){
    if(with<=aval){
        aval-=with;
        cout<<"Withdrawal successful. Available cash: "<<fixed<<setprecision(1)<<aval<<endl;
    }
    else{
